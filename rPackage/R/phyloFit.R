...
#                     ancestor=NULL,
  ...
  if (!inherits(design.matrix, 'matrix'))
      stop("design.matrix must be a matrix")
  if (!all(is.finite(design.matrix)))
     stop("design.matrix must only contain finite values")
  if (nrow(design.matrix) != 12)
      stop("design.matrix must have 12 rows, assuming 4 states in location trait")
  if (mode(design.matrix) != 'numeric')
      stop("design.matrix should be numeric")
  check.arg(npenalties, "npenalties", "integer", null.OK=FALSE) 
