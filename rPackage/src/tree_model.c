...
TreeModel *tm_new(TreeNode *tree, MarkovMatrix *rate_matrix, 
                  ...
                  Matrix *design_mat, int npenalties) {
  ...
  tm->npenalties = npenalties;
  tm->eta_design_matrix = design_mat;
  if (tm->eta_design_matrix != NULL){
    tm->eta_coefficients = vec_new(design_mat->ncols);
    vec_set_all(tm->eta_coefficients, 0);
    vec_set(tm->eta_coefficients, 0, 1);
    tm->beta = mat_new(tm->npenalties, design_mat->ncols);
    mat_set_all(tm->beta, -1);
    tm->penalties = vec_new(tm->npenalties);
  } else {
    tm->eta_coefficients = NULL;
    tm->beta = NULL;
    tm->penalties = NULL;
  }
...
void tm_free(TreeModel *tm) {
  ...
  if (tm->beta != NULL)
    mat_free(tm->beta);
  if (tm->penalties != NULL)
    vec_free(tm->penalties);
  sfree(tm);
}
...
void tm_print(FILE *F, TreeModel *tm) {
  ...
  if (tm->eta_coefficients != NULL) {
    fprintf(F, "%s ", ETA_TAG);
    for (i = 0; i < tm->eta_coefficients->size; i++) 
      fprintf(F, "%f ", vec_get(tm->eta_coefficients, i));
    fprintf(F, "\n");
  }
...
TreeModel *tm_create_copy(TreeModel *src) {
  ...
  retval = tm_new(src->tree != NULL ? tr_create_copy(src->tree) : NULL, 
                  ...
                  src->root_leaf_id, src->eta_design_matrix, src->npenalties);
  retval->msa = src->msa;
  retval->lnL = src->lnL;
...
int tm_fit(TreeModel *mod, MSA *msa, Vector *params, int cat, 
  ...
  retval = opt_bfgs_regression(tm_likelihood_wrapper, opt_params, (void*)mod, &ll,
                               ...
                               beta_params, mod->npenalties, mod->penalties, mod->beta);
  fprintf(stderr, "pens= "); vec_print(mod->penalties, stderr);
  fprintf(stderr, "beta= "); mat_print(mod->beta, stderr);
...
void get_beta_params_direction(Matrix *Binv, void *data, Vector *at_bounds, int params_at_bounds,
                               ...
                               Vector *beta_params, double lambda, double *log_penalty,
  ...
  double beta_cur_k, beta_k_deriv, beta_k_deriv2, penalty;

  mod = (TreeModel*)data;
  beta = beta_params;
  X = mod->eta_design_matrix;
  penalty = exp(*log_penalty);
  ...
  if (logf != NULL) {
    fprintf(logf, "target params:\n");
    vec_print(params_new, logf);
  }
  /*
  printf("\nW:\n");
  mat_print(W, stdout);
  ...
  beta_cur = vec_new(beta->size);
  beta_gradient = vec_new(beta->size);
  beta_hess_diag = vec_new(beta->size);  
  beta_no_k = vec_new(beta->size);
  r = vec_new(B->nrows);
  beta_old = vec_new(beta->size);

  vec_copy(beta_cur, beta);
  do {
    for(k = 0; k < beta->size; k++){
      vec_copy(beta_old, beta_cur);
      vec_copy(beta_no_k, beta_cur);
...
TreeModel *tm_induced_aa(TreeModel *codon_mod) {
  ...
  retval = tm_new(tr_create_copy(codon_mod->tree), aa_mat, aa_freqs, 
                  ...
                  NULL, codon_mod->root_leaf_id, NULL, 1);
