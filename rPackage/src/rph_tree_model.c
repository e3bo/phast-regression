...
SEXP rph_tm_new(SEXP treeP, SEXP alphabetP, SEXP backgdP, SEXP matrixP, 
		...
		SEXP selectionP, SEXP siteModelP, SEXP designMatP, 
                SEXP nPenaltiesP) {
  TreeModel *tm;
  TreeNode *tree;
  MarkovMatrix *rateMatrix;
  Matrix *m, *design_matrix;
  int dim=-1, i, numProtect=0, nratecats=1, rootLeaf, ncoef=-1;
  int j, I, J, npenalties=1;
  double *doubleP, alpha=0.0, tmp;
  Vector *backgd;
  char *alphabet;
  ...
  //regression
  if (designMatP == R_NilValue)
    design_matrix = NULL;
  else 
    design_matrix = rph_get_rect_matrix(designMatP);
  if (nPenaltiesP != R_NilValue)
    npenalties = INTEGER_VALUE(nPenaltiesP);

  tm = tm_new(tree, rateMatrix, backgd, subst_mod,
	      alphabet, nratecats, alpha, rate_consts, 
	      rootLeaf, design_matrix, npenalties);
