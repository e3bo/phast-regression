...
int opt_bfgs_regression(double (*f)(Vector*, void*), Vector *params,
	     ...
	     int *num_evals, double (*freg)(Vector*, void*),
             Vector *beta_params, int npenalties, Vector *penalties, Matrix *beta) {
  ...
  double penalty, hi_penalty, lo_penalty, tmp, penalty_ratio = 0.0001, penstep;
  double size, relaxed_target_size;
  update_params(params, beta_params, data);

  greg = vec_new(nreg);
  beta_direction = vec_new(nreg);
  beta_params_new = vec_new(nreg);
  xi_old = vec_new(n); beta_params_old = vec_new(nreg); params_old = vec_new(n);
  B = mat_new(n, n);
...
/*     vec_set(at_bounds, i, OPT_NO_BOUND); */
  ...
  for (penalty_index = 0; penalty_index < npenalties; penalty_index++){
    ...
    if (do_inner_opt) {
      ...
      if (did_inner_opt || find_hi_penalty) {
        get_beta_params_direction(H, data, at_bounds, params_at_bounds, g,
                                  params_new, beta_direction, beta_params, lambda, 
                                  &penalty, find_hi_penalty, logf);
        if (find_hi_penalty) {
          hi_penalty = penalty;
          lo_penalty = hi_penalty + log(penalty_ratio);
          penstep = (hi_penalty - lo_penalty) / (npenalties - 1);
        }
      /*  opt_lnsrch(beta_params, fval, greg, beta_direction, beta_params_new, retval, stpmax,
        &check, freg, data, &nevals, &lambda, logf);*/
      /*  if(check) lambda = 1; /* assume due to application of soft-threshold for penalty */
          vec_copy(beta_params_new, beta_params);
          vec_plus_eq(beta_params_new, beta_direction);
      } else { /* this will relax the penalty a bit at first to get the iterations moving along */
        size = vec_1norm(beta_params) - vec_get(beta_params, 0);
        if (size == 0) {
          relaxed_target_size = exp(penstep) * vec_get(beta_params, 0);
        } else {
          relaxed_target_size = exp(penstep) * size;
        }
        tmp = penalty;
        while(tmp >= lo_penalty & size < relaxed_target_size) {
          get_beta_params_direction(H, data, at_bounds, params_at_bounds, g,
                                    params_new, beta_direction, beta_params, lambda, 
                                    &tmp, 0, logf);
          vec_copy(beta_params_new, beta_params);
          vec_plus_eq(beta_params_new, beta_direction);
          size = vec_1norm(beta_params_new) - vec_get(beta_params, 0);
          tmp -= penstep;
        }
      }

      if (logf != NULL) {
        fprintf(logf, "beta_params: ");
        vec_print(beta_params, logf);
        fprintf(logf, "beta params new: ");
        vec_print(beta_params_new, logf);
        fprintf(logf, "lambda: %g\n", lambda);      
        fprintf(logf, "fval = %g\n", fval);
        fprintf(logf, "fregval = %g\n", *retval);
      }

      update_params(params_new, beta_params_new, data);
      *retval = f(params_new, data);
      vec_copy(beta_params_old, beta_params);
...
/*       check_H(H, at_bounds); */
    ...
    if(logf != NULL){
      fprintf(logf, "penalty=%g, fval=%g, beta_vec=", penalty, fval); 
      vec_print(beta_params,logf);
    }
    vec_set(penalties, penalty_index, penalty);
    for (i=0; i < beta->ncols; i++){
      tmp = vec_get(beta_params, i);
      mat_set(beta, penalty_index, i, tmp);      
    }
    /*rewind last step */
    vec_copy(beta_params, beta_params_old);
    vec_copy(params, params_old);
    vec_copy(xi, xi_old);
    fval = fval_old;
  }
  ...
  vec_free(params_new);
  vec_free(xi);
  vec_free(xi_old); vec_free(beta_params_old); vec_free(params_old);
  vec_free(at_bounds);
  if (inv_Hessian == NULL) mat_free(H);
  mat_free(first_frac);
  mat_free(sec_frac);
