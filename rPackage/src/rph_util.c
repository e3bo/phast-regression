...
// returns a phast Matrix from an R matrix.  Does NOT assume matrix is square.
Matrix *rph_get_rect_matrix(SEXP matP) {
  double *doubleP;
  Matrix *m;
  int pos, i, j, I, J;
  SEXP dim;
  PROTECT(matP);
  dim = getAttrib(matP, R_DimSymbol);
  I = INTEGER_POINTER(dim)[0];
  J = INTEGER_POINTER(dim)[1];
  matP = AS_NUMERIC(matP);
  doubleP = NUMERIC_POINTER(matP);
  m = mat_new(I, J);
  pos=0;
  for (j=0; j<J; j++)
    for (i=0; i<I; i++)
      mat_set(m, i, j, doubleP[pos++]);
  UNPROTECT(1);
  return m;
}
