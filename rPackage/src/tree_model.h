...
struct tm_struct {
 ...
 /* These are the indices in all_params that show where scale, branchlens, rateMatrix,
  ...
  Vector *penalties;            /* actual penalties used */
  Matrix *beta;                 /* nvars by npenalties matrix of fitted regression coefficient */
  double npenalties;
};
...
TreeModel *tm_new(TreeNode *tree,  /**< Phylogenetic tree */
		  ...
		  int root_leaf_id,     /**< ID number of the root node */
                  Matrix *design_mat,       /*design matrix for linear predictors*/
                  int npenalties            /*number of points in the regularization path*/
		  );
